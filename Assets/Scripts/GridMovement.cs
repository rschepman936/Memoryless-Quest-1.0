            ...
            if (Input.GetKeyDown(KeyCode.W)||Input.GetKeyDown(KeyCode.A)||Input.GetKeyDown(KeyCode.S)||Input.GetKeyDown(KeyCode.D)){
                ...
                if(Vector3.Distance(transform.position, movePoint.position) <= 0.5f){
                    ...
                    if  (Mathf.Abs(Input.GetAxisRaw("Horizontal")) == 1f ){
                        ...
                        if(Physics2D.OverlapCircle(movePoint.position + new Vector3(Input.GetAxisRaw("Horizontal"),0f,0f),.2f,whereCanMove)){
                            movePoint.position +=  new Vector3(Input.GetAxisRaw("Horizontal"),0f,0f);
                            playersTurn.isPlayerTurn=false;
                        }
                    }else if  (Mathf.Abs(Input.GetAxisRaw("Vertical")) == 1f ){
                        if(Physics2D.OverlapCircle(movePoint.position + new Vector3(0f,Input.GetAxisRaw("Vertical"),0f),.2f,whereCanMove)){
